...
for company in list_companies:
    ...
    if len(files) > 0:
         #Create QR code
        http = f"http://{ip}:8501/?Company={company}&Product_ID=1"
        lib.create_qr(path_image, files[0], http, f"{company}_qr1.png")
        with columns[list_companies.index(company)].container():
            ...
            if st.button(company):
                ...
                global_state['status'] = 'not OK' 
                streamlit_js_eval(js_expressions="parent.window.location.reload()")
            st.image(os.path.join(path_image,f"{company}_qr1.png"), caption=f"{company} Test with your phone", width=200)
